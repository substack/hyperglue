...
attributes on all matching elements with the keys in the target values.

These attributes are special:
* `_html` - set the innerHTML in object form. If `'_html'` is an HTML element,
replace the inner content at the selector elements with a clone of the `'_html'` value.
* `_text` - set the text content encoded as html entities
* `_append`, `_appendText` - add text to the end of the content encoded as
html entities
* `_appendHtml` - add raw html to the end of the content
* `_prepend`, `_prependText` - add text to the beginning of the content
encoded as html entities
* `_prependHtml` - add raw html to the beginning of the content
